...
(function(wysihtml) {
    ...
    /** @scope wysihtml.toolbar.Toolbar.prototype */ {
      constructor: function(editor, container, showOnInit) {
        ...
        var speechInputLinks = this.container.querySelectorAll(
            '[data-wysihtml-command=insertSpeech]'
          ),
      ...
      _getLinks: function(type) {
        var links = (this[type + 'Links'] = wysihtml.lang
            .array(
              this.container.querySelectorAll('[data-wysihtml-' + type + ']')
            )
            .get()),
          length = links.length,
          i = 0,
        ...
        for (; i < length; i++) {
          ...
          tracksBlankValue = link.getAttribute(
            'data-wysihtml-' + type + '-blank-value'
          );
          group = this.container.querySelector(
            '[data-wysihtml-' + type + "-group='" + name + "']"
          );
      ...
      _getDialog: function(link, command) {
        ...
        if (dialogElement) {
          if (wysihtml.toolbar['Dialog_' + command]) {
            dialog = new wysihtml.toolbar['Dialog_' + command](
              link,
              dialogElement
            );
          } else {
      ...
      _observe: function() {
        ...
        dom.delegate(container, '[data-wysihtml-command]', 'click', function(
          event
        ) {
          var state,
            link = this,
        ...
        dom.delegate(container, '[data-wysihtml-action]', 'click', function(
          event
        ) {
        ...
        this._ownerDocumentClick = function(event) {
          if (
            !wysihtml.dom.contains(that.container, event.target) &&
            !wysihtml.dom.contains(that.composer.element, event.target)
          ) {
            that._updateLinkStates();
            that._preventInstantFocus();
          }
        ...
        if (this.editor.config.handleTables) {
          editor.on('tableselect:composer', function() {
            that.container.querySelectorAll(
              '[data-wysihtml-hiddentools="table"]'
            )[0].style.display =
              '';
          });
        ...
        for (i in commandMapping) {
          ...
          if (state) {
            if (command.tracksBlankValue) {
              ...
              if (
                command.dialog &&
                (typeof command.value === 'undefined' || command.value === null)
              ) {
                if (state && typeof state === 'object') {
                  ...
                  displayDialogAttributeValue = command.dialog.container.dataset
                    ? command.dialog.container.dataset.showdialogonselection
                    : false;

                  if (
                    composer.config.showToolbarDialogsOnSelection ||
                    displayDialogAttributeValue
                  ) {
  ...
  wysihtml.extendEditor(function(editor) {
    if (editor.config.toolbar) {
      editor.toolbar = new wysihtml.toolbar.Toolbar(
        editor,
        editor.config.toolbar,
        editor.config.showToolbarAfterInit
      );
